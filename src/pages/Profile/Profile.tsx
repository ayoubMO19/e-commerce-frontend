...
import { User, Mail, ShieldCheck, LogOut, Loader2, Save, KeyRound, BadgeCheck } from "lucide-react";
...
export default function Profile() {
    ...
    const updateMutation = useMutation({
        mutationFn: (data: { name: string; surname: string }) => userService.update(data),
    ...
    // Si el usuario se desloguea mientras está en esta página, retornamos null
    if (!user) return null;
                    ...
                    <main className="space-y-8 lg:col-span-8">
                        <section className="rounded-[2.5rem] border border-gray-100 bg-white p-8 shadow-sm sm:p-12">
                            <div className="mb-10 flex items-center gap-4">
                                ...
                                <div>
                                    <h3 className="text-sm font-black uppercase tracking-widest text-gray-900">Información Personal</h3>
                                    <p className="text-xs text-gray-400">Actualiza tu identidad pública en la plataforma.</p>
                                </div>
                            </div>
                            ...
                            <form onSubmit={handleUpdateProfile} className="space-y-8">
                                <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
                                    ...
                                    <div className="space-y-2">
                                        <label className="ml-1 text-[10px] font-black uppercase tracking-widest text-gray-400">Apellidos</label>
                                        <input
                                            type="text"
                        ...
                        <section className="rounded-[2.5rem] border border-gray-100 bg-white p-8 shadow-sm sm:p-12">
                            <div className="flex flex-col items-start justify-between gap-8 sm:flex-row sm:items-center">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <h3 className="text-sm font-black uppercase tracking-widest text-gray-900">Seguridad</h3>
                                        <p className="text-xs text-gray-400">Recibirás un enlace seguro para cambiar la clave.</p>
                                    </div>
                                </div>
